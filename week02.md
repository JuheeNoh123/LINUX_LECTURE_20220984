...
```
설치가 완료되면 초기 아이디/비밀번호를 설정하고 셸에 진입한다.  
WSL Settings에서 CPU 코어 수, 메모리, 스왑 크기, 네트워크 모드 등을 조정할 수 있으며, 저사양 환경에서는 필수적으로 점검하는 것이 좋다.
...
docker run -d \
  --name my-nginx \
  -p 8080:80 \
  --mount type=bind,source=./www,target=/usr/share/nginx/html \
  nginx:latest
```
